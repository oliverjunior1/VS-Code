...
--3. Ticket médio.
SELECT cliente, SUM(valor)/COUNT(cliente) AS ticket_por_cliente FROM clientes_vendas GROUP BY cliente;

--4. Clientes que gastaram mais de 300.
-- Minha solução
SELECT cliente, SUM(valor) AS total_compras, SUM(valor)>300 AS gastou_mais_que_300 FROM clientes_vendas GROUP BY cliente;
-- solução chatgpt
SELECT cliente,
